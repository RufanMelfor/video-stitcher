...
## Pre-label teacher model comparison (2026-08-09, RUFAN_LAPTOP session)

Side-quest ahead of the *next* fine-tuning round (doesn't touch the
review/dataset work above): before defaulting to yolo26x as the
pre-label/teacher model for a future `export_yolo_labels.py`-style run,
ran a visual side-by-side of 4 candidate teacher models on the same 150
frames (evenly sampled across the full ~20min length of a drone test
video, left camera), each pushed into its own Label Studio project via
the REST API:

- `Adit-jain/soccana` (yolo11n, football-trained, player/ball/referee)
- `martinjolif/yolo-football-player-detection` (yolo11m, football-
  trained, 4-class incl. goalkeeper)
- `martinjolif/yolo-football-ball-detection` (yolo11n, ball-only
  specialist)
- stock `yolo26x.pt` (COCO-pretrained, not football-trained, filtered to
  person/ball to match this project's own convention)

**User's verdict: soccana looked best of all four, beating even stock
yolo26x** - domain-specific football training outweighed yolo26x's
newer/larger architecture on this footage. The ball-only specialist was
weakest, missing the ball in 94/150 frames - specialization alone didn't
help without football-diverse training data. Practical implication:
prefer `Adit-jain/soccana` (or similar football-trained yolo11) as the
pre-label/teacher model instead of yolo26x next time - class-name remap
still needed at export time (soccana's player+referee -> person), same
as already done for stock COCO.

Also connected a *live* Label Studio ML backend to the soccana project
(not just static batch predictions): a small Flask app using the
`label-studio-ml` SDK (`LabelStudioMLBase` subclass wrapping the model),
running locally on this laptop, registered via the ML backend API -
shows "Connected" in the project's Model tab and returns live
predictions on demand instead of only the pre-uploaded static ones.
Needed a Windows Defender Firewall inbound rule for the backend's port -
fails with "Toegang geweigerd"/access denied unless PowerShell is run
elevated (Administrator). Only live while this laptop and that Python
process stay running.

Infra notes for whoever's on the Pi next:
- The Pi's ZeroTier route was broken this session (ICMP replied, every
  TCP port timed out, root cause not diagnosed) - its plain LAN address
  worked directly instead (same subnet as this laptop). See password
  manager / `zerotier-cli listnetworks` for either address, not recorded
  here.
- No SSH key set up to the Pi from this laptop (password auth only), so
  the usual rsync-onto-`LOCAL_FILES_DOCUMENT_ROOT` workflow wasn't
  available - worked around it by uploading images straight through
  Label Studio's own REST file-upload API instead (`POST
  /api/projects/<id>/import` with the file as multipart), no rsync/SSH
  needed at all.
- Gotcha: that import endpoint's response does *not* include `task_ids`
  in this LS version (1.23.0) despite creating the tasks fine - fetch
  `GET /api/tasks?project=<id>` afterwards and match by filename
  (`/data/upload/<project>/<hash>-<original_filename>`) to get real task
  ids back before calling `POST /api/predictions/`.
