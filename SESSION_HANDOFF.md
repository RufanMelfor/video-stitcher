...
7. CVAT set up and working locally on this PC:
   ...
   - Also: `cvat-cli task create ... local <directory>` does not expand
     ...
     PermissionError.
8. Now in progress: getting CVAT running on the user's Synology NAS too
   (Container Manager, Docker-capable), so it's reachable from multiple
   workstations instead of only this PC. See "Immediate blocker" above.
   Also discussed: storing the raw dataset files (not CVAT's own
   internal storage) on the NAS via a WebDAV mount for cross-workstation
   access - not done yet, waiting on the NAS-side CVAT setup first.
