...
impl Lens {
    ...
    /// [`Topology::ground_tilt_z`]'s `warp_ground_y(t, c, k)` (see
...
pub struct Topology {
    ...
    /// rendering choice - doesn't move the seam or touch the plane geometry.
    ...
    /// local UV units (same space as `blend_width`). `0.0` (default,
    /// no-op) = seam sits exactly where the calibrated geometry puts it.
    ///
    /// Deliberately *not* [`Self::intersect`]: `intersect` repositions the
    /// plane geometry itself, which changes how much the two cameras'
    ...
    /// `ViewportConfig::color_match_interval_frames`.
    ...
    /// `ViewportConfig::color_match_ema_alpha`.
    ...
    /// `ViewportConfig::color_match_max_y_offset`.
    ...
    /// `ViewportConfig::color_match_max_chroma_offset`.
    ...
    /// Ground-plane tilt correction for the z-plane (`lenses[0]`'s content).
    /// See [`Self::ground_tilt_x`] - same math, applied to the other plane.
    #[serde(default)]
    pub ground_tilt_z: f64,

    /// Top-of-frame tilt correction for the x-plane (`lenses[1]`'s content).
    ...
    /// this yet); set by hand via the calibration UI or the calibration
    /// file. `0.0` (the default) is a no-op.
    #[serde(default)]
    pub top_tilt_x: f64,

    /// Top-of-frame tilt correction for the z-plane (`lenses[0]`'s content).
...
/// UV units as `blend_width`. Wide enough to give a visually meaningful
/// nudge, narrow enough to stay inside the geometric overlap margin for
/// typical calibrations (unlike [`Topology::intersect`], this doesn't adapt
/// to how much overlap a given calibration actually has, so an extreme drag
/// can still walk the seam into an uncovered region on a tightly-calibrated
/// rig - the same failure mode as `intersect`, just harder to reach).
...
/// a calibration written before they existed. Also the engineering defaults
/// a consumer GUI should offer a "reset" control for.
pub const DEFAULT_COLOR_MATCH_ENABLED: bool = true;
/// See [`DEFAULT_COLOR_MATCH_ENABLED`].
pub const DEFAULT_COLOR_MATCH_BAND_WIDTH: f32 = 0.15;
/// See [`DEFAULT_COLOR_MATCH_ENABLED`].
pub const DEFAULT_COLOR_MATCH_GRID_COLS: u32 = 8;
/// See [`DEFAULT_COLOR_MATCH_ENABLED`].
pub const DEFAULT_COLOR_MATCH_GRID_ROWS: u32 = 16;
/// See [`DEFAULT_COLOR_MATCH_ENABLED`].
pub const DEFAULT_COLOR_MATCH_INTERVAL_FRAMES: u32 = 15;
/// See [`DEFAULT_COLOR_MATCH_ENABLED`].
pub const DEFAULT_COLOR_MATCH_EMA_ALPHA: f32 = 0.15;
/// See [`DEFAULT_COLOR_MATCH_ENABLED`].
...
fn default_color_match_band_width() -> f32 {
    DEFAULT_COLOR_MATCH_BAND_WIDTH
}
fn default_color_match_grid_cols() -> u32 {
    DEFAULT_COLOR_MATCH_GRID_COLS
}
fn default_color_match_grid_rows() -> u32 {
    DEFAULT_COLOR_MATCH_GRID_ROWS
}
fn default_color_match_interval_frames() -> u32 {
    DEFAULT_COLOR_MATCH_INTERVAL_FRAMES
}
fn default_color_match_ema_alpha() -> f32 {
    DEFAULT_COLOR_MATCH_EMA_ALPHA
}
...
/// Inclusive bound for [`Topology::ground_tilt_x`]/`ground_tilt_z`/
/// [`Topology::top_tilt_x`]/`top_tilt_z`.
pub const TILT_RANGE: f64 = 0.3;
/// Inclusive bounds for [`Topology::ground_tilt_band_width`]/
/// [`Topology::top_tilt_band_width`].
...
fn validate_topology(t: &Topology) -> Result<(), CalibrationError> {
    ...
    if !SEAM_OFFSET_RANGE.contains(&t.seam_offset) {
        return Err(CalibrationError::OutOfRange {
            field: "topology.seam_offset".to_owned(),
            value: t.seam_offset as f64,
            min: *SEAM_OFFSET_RANGE.start() as f64,
            max: *SEAM_OFFSET_RANGE.end() as f64,
        });
    }

    for (name, val) in [
        ("topology.color_match_band_width", t.color_match_band_width),
        ("topology.color_match_ema_alpha", t.color_match_ema_alpha),
        (
            "topology.color_match_max_y_offset",
            t.color_match_max_y_offset,
        ),
        (
            "topology.color_match_max_chroma_offset",
            t.color_match_max_chroma_offset,
        ),
    ] {
        if !val.is_finite() {
            return Err(CalibrationError::NonFiniteFloat {
...
mod tests {
    ...
    fn json_round_trips() {
        ...
        cal.topology.blend_flip_direction = true;
        cal.topology.seam_offset = -0.077;
        cal.topology.multiband_blend_enabled = true;
        cal.topology.ground_tilt_x = -0.09;
        cal.topology.ground_tilt_z = -0.085;
        cal.topology.top_tilt_x = 0.05;
        cal.topology.top_tilt_z = -0.04;
        cal.topology.ground_tilt_band_width = 0.2;
        cal.topology.top_tilt_band_width = 0.18;
        ...
        assert!(back.topology.blend_flip_direction);
        assert!((back.topology.seam_offset + 0.077).abs() < 1e-6);
        assert!(back.topology.multiband_blend_enabled);
        assert!((back.topology.ground_tilt_x + 0.09).abs() < 1e-9);
        assert!((back.topology.ground_tilt_z + 0.085).abs() < 1e-9);
        assert!((back.topology.top_tilt_x - 0.05).abs() < 1e-9);
        assert!((back.topology.top_tilt_z + 0.04).abs() < 1e-9);
        assert!((back.topology.ground_tilt_band_width - 0.2).abs() < 1e-9);
        assert!((back.topology.top_tilt_band_width - 0.18).abs() < 1e-9);
    ...
    fn ground_tilt_k_matches_documented_formula() {
        ...
        assert!((lens.ground_tilt_k() - 0.1897).abs() < 1e-4);
    }
