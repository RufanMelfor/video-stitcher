...
//! Auto-Calibrate run.
//!
//! Unlike the auto-calibrate job, which spins up a fresh `GpuContext` per
//! run, this worker creates its GPU context and per-resolution
//! `GpuUndistort` pipelines once and reuses them for the life of the app -
//! it fires on every slider tweak and has to stay cheap.
//!
...
impl DetectPreviewWorker {
    ...
    /// state (same convention as the auto-calibrate background job).
