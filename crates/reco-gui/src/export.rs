...
pub struct AutocamUiConfig {
    ...
    /// EXPERIMENTAL: run the detector's inference call on a dedicated
    /// background thread instead of blocking the export loop (see
...
pub fn run_export(
    ...
    if autocam.enabled && !autocam.model_path.is_empty() {
        ...
        job = job.on_session(move |session, source| {
            ...
            // EXPERIMENTAL: --async-detect equivalent. Only meaningful
            // once tracking is confirmed active and the buffered/export
            // loop is in play (lookahead > 0) - a second, separate
            // detector instance moved onto reco-core's async worker
            ...
            if matches!(result, Ok(true)) && ac.async_detect && ac.lookahead_secs > 0.0 {
                match reco_autocam::CpuYoloDetector::with_config(
                    ...
                    Ok(inference_detector) => {
                        ...
                        log::info!(
                            "Export: EXPERIMENTAL async detect thread active (queue depth {queue_depth})"
                        );
                    }
                    Err(e) => log::warn!(
