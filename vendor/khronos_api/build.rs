...
fn main() {
    ...
    println!("cargo:warning=khronos_api build script running, OUT_DIR={dest}");
    ...
    // containing file.
    // Use CARGO_MANIFEST_DIR instead of current_dir() — Cargo guarantees this
    // variable points to the package source root regardless of working directory.
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").unwrap();
    let root = Path::new(&manifest_dir).join("api_webgl/extensions");
